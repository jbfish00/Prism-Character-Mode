#!/usr/bin/env python3
"""Scrape each classic character's documented Pokemon from Bulbapedia.

Ported verbatim (algorithm unchanged) from the ROWE/Unbound/RadicalRed lineage's
scrape_rosters.py. Only change: characters.txt now has a 5th `source` field, so
this script filters to source=bulbapedia and leaves source=rijon entries alone
(see scrape_rijon_wiki.py for those). Output merges into the same rosters_raw.json
both scrapers share.
...
Usage: python3 tools/character_mode/scrape_bulbapedia.py [--only "Name"]
"""
import json
import os
import re
...
def main():
    ...
    chars = load_characters("bulbapedia")
    ...
    for disp, pages, cat, gen in chars:
        ...
        out[disp] = {"page": " + ".join(pages), "category": cat, "gen": gen,
                     "source": "bulbapedia", "species": sorted(species)}
    ...
    if problems:
        print("\n--- problems ---")
        print("\n".join(problems))
    print("\nwrote %s (%d characters total, bulbapedia pass)" % (out_path, len(out)))

#!/usr/bin/env python3
"""Minimal SM83 (GBZ80) linear disassembler for probing Prism ROM regions.

Usage:
  python3 gbdis.py <rom.gbc> <file_offset_hex> [count]

Prints one instruction per line with file offset and in-bank address.
Linear sweep only (no control-flow tracing) - good enough for eyeballing
routine boundaries around a known point of interest.
"""
import sys
...
def decode(rom, off):
    """Return (length, text) for the instruction at rom[off]."""
    op = rom[off]
...
def main():
    ...
    for _ in range(count):
        ...
        print(f"{off:06X}  {bank:02X}:{addr:04X}  {raw:<8}  {text}")
        off += n

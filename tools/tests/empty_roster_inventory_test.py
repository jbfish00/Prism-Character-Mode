#!/usr/bin/env python3
"""Negative test for emit_rosters.py's EMPTY_ROSTER_EXPECTED inventory.
...
spec = importlib.util.spec_from_file_location(
    "emit_rosters", os.path.join(CM, "emit_rosters.py"))
ec = importlib.util.module_from_spec(spec)
sys.modules["emit_rosters"] = ec
...
def main():
    ...
    # pinning a fiction. Prism has no build_rosters(); its empty flag is simply
    # "no species survived mapping", so derive it the same way main() does.
    with open(os.path.join(CM, "rosters_mapped.json")) as f:
        mapped = json.load(f)
    live = sorted(c for c, v in mapped.items()
                  if not [e for e in v.get("species", [])
                          if e.get("species_id") is not None])
    if set(live) != real:
